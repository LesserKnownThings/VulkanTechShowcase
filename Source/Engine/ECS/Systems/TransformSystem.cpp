...
void TransformSystem::HandleEntityRemoved(const Entity &entity)
{
    ...
    if (it != instances.end())
    {
        component.isAlive[it->second] = false;
...
{
    const uint32_t cID = instances[entity];
    component.eulers[cID] += glm::vec3(angle * axis.x, angle * axis.y, angle * axis.z);
    RecalculateModel(cID);
}
...
void TransformSystem::Allocate(int32_t size)
{
    ...
    newComp.buffer = malloc(size * (sizeof(Entity) + sizeof(glm::mat4) + sizeof(glm::vec3) * 3 + sizeof(uint8_t) * 2));
    ...
    newComp.isStatic = (uint8_t *)(size + newComp.entity);
    newComp.isAlive = size + newComp.isStatic;
    ...
    memcpy(newComp.isAlive, component.isAlive, component.instances * sizeof(uint8_t));
