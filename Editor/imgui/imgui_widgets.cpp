...
void ImGui::TextEx(const char* textWidget, const char* text_end, ImGuiTextFlags flags)
{
    ...
    // Accept null ranges
    if (textWidget == text_end)
        textWidget = text_end = "";

    // Calculate length
    const char* text_begin = textWidget;
    if (text_end == NULL)
        text_end = textWidget + strlen(textWidget); // FIXME-OPT

    const ImVec2 text_pos(window->DC.CursorPos.x, window->DC.CursorPos.y + window->DC.CurrLineTextBaseOffset);
    const float wrap_pos_x = window->DC.TextWrapPos;
    const bool wrap_enabled = (wrap_pos_x >= 0.0f);
    if (text_end - textWidget <= 2000 || wrap_enabled)
    {
        // Common case
...
void ImGui::TextUnformatted(const char* textWidget, const char* text_end)
{
    TextEx(textWidget, text_end, ImGuiTextFlags_NoWidthForLargeClippedText);
}
...
void ImGui::TextV(const char* fmt, va_list args)
{
    ...
    const char* textWidget, *text_end;
    ImFormatStringToTempBufferV(&textWidget, &text_end, fmt, args);
    TextEx(textWidget, text_end, ImGuiTextFlags_NoWidthForLargeClippedText);
}
...
void ImGui::ShrinkWidths(ImGuiShrinkWidthItem* items, int instanceCount, float width_excess)
{
    if (instanceCount == 1)
    {
    ...
    ImQsort(items, (size_t)instanceCount, sizeof(ImGuiShrinkWidthItem), ShrinkWidthItemComparer);
    int count_same_width = 1;
    while (width_excess > 0.0f && count_same_width < instanceCount)
    {
        while (count_same_width < instanceCount && items[0].Width <= items[count_same_width].Width)
            count_same_width++;
        float max_width_to_remove_per_item = (count_same_width < instanceCount && items[count_same_width].Width >= 0.0f) ? (items[0].Width - items[count_same_width].Width) : (items[0].Width - 1.0f);
    ...
    while (width_excess > 0.0f)
        for (int n = 0; n < instanceCount && width_excess > 0.0f; n++)
        {
...
{
    const char* textWidget = obj->TextSrc;
    const char* text_remaining = NULL;
    const ImVec2 size = InputTextCalcTextSize(obj->Ctx, textWidget + line_start_idx, textWidget + obj->TextLen, &text_remaining, NULL, true);
    r->x0 = 0.0f;
    r->x1 = size.x;
    r->baseline_y_delta = size.y;
    r->ymin = 0.0f;
    r->ymax = size.y;
    r->num_chars = (int)(text_remaining - (textWidget + line_start_idx));
}
...
static bool STB_TEXTEDIT_INSERTCHARS(ImGuiInputTextState* obj, int pos, const char* new_text, int new_text_len)
{
    ...
    char* textWidget = obj->TextA.Data;
    if (pos != text_len)
        memmove(textWidget + pos + new_text_len, textWidget + pos, (size_t)(text_len - pos));
    memcpy(textWidget + pos, new_text, (size_t)new_text_len);
...
static void stb_textedit_replace(ImGuiInputTextState* str, STB_TexteditState* state, const IMSTB_TEXTEDIT_CHARTYPE* textWidget, int text_len)
{
    ...
    if (text_len <= 0)
        return;
    if (ImStb::STB_TEXTEDIT_INSERTCHARS(str, 0, textWidget, text_len))
    {
...
void ImGui::ColorTooltip(const char* textWidget, const float* col, ImGuiColorEditFlags flags)
{
    ImGuiContext& g = *GImGui;

    if (!BeginTooltipEx(ImGuiTooltipFlags_OverridePrevious, ImGuiWindowFlags_None))
        return;
    const char* text_end = textWidget ? FindRenderedTextEnd(textWidget, NULL) : textWidget;
    if (text_end > textWidget)
    {
        TextEx(textWidget, text_end);
        Separator();
    }
...
bool ImGui::BeginMenuEx(const char* label, const char* iconWidget, bool enabled)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    else
    {
        ...
        float icon_w = (iconWidget && iconWidget[0]) ? CalcTextSize(iconWidget, NULL).x : 0.0f;
        ...
        RenderText(text_pos, label);
        if (icon_w > 0.0f)
            RenderText(pos + ImVec2(offsets->OffsetIcon, 0.0f), iconWidget);
...
bool ImGui::MenuItemEx(const char* label, const char* iconWidget, const char* shortcut, bool selected, bool enabled)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
