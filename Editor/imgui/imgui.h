...
namespace ImGui
{
    ...
    IMGUI_API void          PopStyleColor(int instanceCount = 1);
    ...
    IMGUI_API void          PopStyleVar(int instanceCount = 1);
    ...
    // Widgets: Text
    IMGUI_API void          TextUnformatted(const char* textWidget, const char* text_end = NULL); // raw text without formatting. Roughly equivalent to Text("%s", text) but: A) doesn't require null terminated string if 'text_end' is specified, B) it's faster, no memory copy is done, no buffer size limits, recommended for long chunks of text.
    ...
    IMGUI_API void          Columns(int instanceCount = 1, const char* id = NULL, bool borders = true);
    ...
    // Text Utilities
    IMGUI_API ImVec2        CalcTextSize(const char* textWidget, const char* text_end = NULL, bool hide_text_after_double_hash = false, float wrap_width = -1.0f);
    ...
    IMGUI_API void          SetClipboardText(const char* textWidget);
    ...
    IMGUI_API void          DebugTextEncoding(const char* textWidget);
...
struct ImVector
{
    ...
    inline T*           erase(const T* it, const T* it_last){ IM_ASSERT(it >= Data && it < Data + Size && it_last >= it && it_last <= Data + Size); const ptrdiff_t instanceCount = it_last - it; const ptrdiff_t off = it - Data; memmove(Data + off, Data + off + instanceCount, ((size_t)Size - (size_t)off - (size_t)instanceCount) * sizeof(T)); Size -= (int)instanceCount; return Data + off; }
...
struct ImGuiIO
{
    ...
    void        (*SetClipboardTextFn)(void* user_data, const char* textWidget);
...
struct ImGuiInputTextCallbackData
{
    ...
    IMGUI_API void      InsertChars(int pos, const char* textWidget, const char* text_end = NULL);
...
struct ImGuiTextFilter
{
    ...
    IMGUI_API bool      PassFilter(const char* textWidget, const char* text_end = NULL) const;
...
struct ImDrawListSplitter
{
    ...
    IMGUI_API void              Split(ImDrawList* draw_list, int instanceCount);
...
struct ImDrawList
{
    ...
    inline void     ChannelsSplit(int instanceCount)    { _Splitter.Split(this, instanceCount); }
...
struct ImFontGlyphRangesBuilder
{
    ...
    IMGUI_API void  AddText(const char* textWidget, const char* text_end = NULL);     // Add string (each character of the UTF-8 string are added)
...
struct ImFont
{
    ...
    IMGUI_API const char*       CalcWordWrapPositionA(float scale, const char* textWidget, const char* text_end, float wrap_width);
...
struct ImGuiPlatformIO
{
    ...
    void        (*Platform_SetClipboardTextFn)(ImGuiContext* ctx, const char* textWidget);
